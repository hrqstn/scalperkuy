...
def round_trip_cost_bps(paper: PaperTradingConfig) -> Decimal:
    return (Decimal(str(paper.fee_rate_bps)) + Decimal(str(paper.slippage_bps))) * Decimal("2")


def required_move_bps(paper: PaperTradingConfig) -> Decimal:
    return max(
        Decimal(str(paper.min_expected_move_bps)),
        round_trip_cost_bps(paper) + Decimal(str(paper.cost_buffer_bps)),
    )


def baseline_long_signal(
    ...
    latest = frame.iloc[-1]
    latest_range_bps = Decimal("0")
    if latest["open"] > 0:
        latest_range_bps = Decimal(str(abs(latest["high"] - latest["low"]) / latest["open"] * 10000))
    cost_bps = round_trip_cost_bps(paper)
    move_floor_bps = required_move_bps(paper)
    ...
    features = {
        ...
        "latest_range_bps": float(latest_range_bps),
        "round_trip_cost_bps": float(cost_bps),
        "required_move_bps": float(move_floor_bps),
    }
    if spread_bps > max_spread_bps:
        return Signal("SKIP", None, Decimal("0"), "spread too wide", features)
    if Decimal(str(paper.take_profit_bps)) < move_floor_bps:
        return Signal("SKIP", None, Decimal("0"), "take profit below required net edge", features)
    if latest_range_bps < move_floor_bps:
        return Signal("SKIP", None, Decimal("0"), "candle range too small to clear costs", features)
...
def micro_momentum_burst_signal(feature: dict, quote: dict, paper: PaperTradingConfig, risk: RiskConfig) -> Signal:
    features = {
        ...
        "is_tradeable_minute": feature.get("is_tradeable_minute"),
        ...
        "round_trip_cost_bps": float(round_trip_cost_bps(paper)),
        "required_move_bps": float(required_move_bps(paper)),
    }

    spread_bps = max(
        Decimal(str(quote.get("spread_bps") or 0)),
        Decimal(str(feature.get("spread_bps_avg") or 0)),
    )
    ...
    quality_score = Decimal(str(feature.get("quality_score") or 0))
    is_tradeable_minute = bool(feature.get("is_tradeable_minute"))
    move_floor_bps = required_move_bps(paper)
    minimum_volatility_bps = max(Decimal(str(paper.min_volatility_bps)), move_floor_bps)
    ...
    if not is_tradeable_minute:
        return Signal("SKIP", None, Decimal("0"), "feature minute not tradeable", features)
    if quality_score < Decimal(str(paper.min_quality_score)):
        return Signal("SKIP", None, Decimal("0"), "feature quality below threshold", features)
    ...
    if Decimal(str(paper.take_profit_bps)) < move_floor_bps:
        return Signal("SKIP", None, Decimal("0"), "take profit below required net edge", features)
    if volatility < minimum_volatility_bps:
